...
class Graph
{
    ...
    // Distance for searches in preprocess phase
...
public:
    ...
    void preprocess() {
        ...
        while (!queue.empty()) {
            std::pair<int, int> dist_pair = queue.top();
            std::vector<Shortcut> shortcuts; 
            int updated_importance = do_shortcut(dist_pair.second, shortcuts, dist_pair.first);
            if ({updated_importance, dist_pair.second} > queue.peek()) { // if importance changed
                queue.update(dist_pair.second, updated_importance); // put it back
            } else {
                for (Shortcut sc: shortcuts) {
                    sc 
                }
                // contract the node by adding the shortcuts
            }
        }
        //    
...
private:
    ...
    void finalize() {
        // Remove unnecessary edges // i.e. the ones going from top to bottom
    }
...
int main() {
    ...
    for (int i = 0; i < t; ++i) {
        ...
        printf("%d\n", g.query(u-1, v-1)); // originally (u-1, v-1, 3)
    }
}
